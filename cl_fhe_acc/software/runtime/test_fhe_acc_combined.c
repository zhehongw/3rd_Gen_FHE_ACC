...
//////////////////////////////////////////////
	...
	rc = BAR1_ROU_table_2k_wr(slot_id);
	fail_on(rc, out, "BAR1 ROU/iROU write 2k failed");
	
	printf("\n===== RLWESUBS test =====\n");
	rc = rlwesubs_dual_test(slot_id);
	fail_on(rc, out, "RLWESUBS test failed");
	...
	printf("\n===== BOOTSTRAP_INIT test =====\n");
	rc = bootstrap_init_test(slot_id);
	fail_on(rc, out, "BOOTSTRAP_INIT test failed");
	...
	TOP_FIFO_MODE 	= RLWEMODE;
    printf("\n===== Switch input FIFO mode =====\n");
	rc = OCL_config_wr_one_addr(slot_id, ADDR_TOP_FIFO_MODE, TOP_FIFO_MODE);
    fail_on(rc, out, "OCL config write failed");

	uint32_t rd_data;
	rc = OCL_config_rd_one_addr(slot_id, ADDR_TOP_FIFO_MODE, &rd_data);
    fail_on(rc, out, "OCL config read failed");
    printf("TOP FIFO MODE readout value: 0x%x\n", rd_data);

    printf("\n===== RLWE MULT RGSW test =====\n");
	rc = rlwe_mult_rgsw_test(slot_id);
    fail_on(rc, out, "RLWE MULT RGSW test failed");
