...
//////////////////////////////////////////////////////
	...
	for(int i = 0; i < 20; i++){
	rc = test::fpga_homo_expansion_and_tree_selection_test(RLWE_uni_dist, RLWE_norm_dist, RLWE_sk, slot_id);

		if(rc != 0){
			std::cout << "FPGA homo expansion and tree selection failed" << std::endl;
			if(ROU_table != NULL)
				free(ROU_table);
			if(iROU_table != NULL)
				free(iROU_table);
