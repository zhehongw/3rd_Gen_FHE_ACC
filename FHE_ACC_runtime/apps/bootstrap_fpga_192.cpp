...
/////////////////////////////////////////////////////
	...
	//BARRETT_M 		= 0x0040000000012FFF;	//for 54 bit
	BARRETT_M 		= 0x000000200001FFFF;	//for 37 bit
...
//////////////////////////////////////////////////////
    ...
    printf("\n===== BAR1 ROU/iROU write 2k =====\n");
	rc = FPGA::BAR1_ROU_table_2k_wr(slot_id);
	if(rc != 0){
		std::cout << "BAR1 ROU/iROU write 2k failed" << std::endl;
		if(ROU_table != NULL)
			free(ROU_table);
		if(iROU_table != NULL)
			free(iROU_table);
	...
	rc = FPGA::dma_write_bootstrap_key(slot_id, bt_key);
	if(rc != 0){
		std::cout << "Bootstrap key transfer failed." << std::endl;
		if(ROU_table != NULL)
			free(ROU_table);
		if(iROU_table != NULL)
			free(iROU_table);
	...
	rc = FPGA::dma_read_compare_bootstrap_key(slot_id, bt_key);
	if(rc != 0){
		std::cout << "Bootstrap key read and compare failed." << std::endl;
		if(ROU_table != NULL)
			free(ROU_table);
		if(iROU_table != NULL)
			free(iROU_table);
	...
	for(int i = 0; i < 10; i++){
		...
		//rc = test::fpga_bootstrapping_test(LWE_uni_dist, LWE_norm_dist, LWE_sk, ks_key, slot_id);
		//cout << "------------------x4-------------------" << endl;
		//rc = test::fpga_bootstrapping_test_x4(LWE_uni_dist, LWE_norm_dist, LWE_sk, ks_key, slot_id);
		//cout << "------------------x8-------------------" << endl;
		//rc = test::fpga_bootstrapping_test_x8(LWE_uni_dist, LWE_norm_dist, LWE_sk, ks_key, slot_id);
		cout << "------------------x12-------------------" << endl;
		rc = test::fpga_bootstrapping_test_x12(LWE_uni_dist, LWE_norm_dist, LWE_sk, ks_key, slot_id);
		//cout << "------------------x10-------------------" << endl;
		//rc = test::fpga_bootstrapping_test_x16(LWE_uni_dist, LWE_norm_dist, LWE_sk, ks_key, slot_id);
		//auto after_bs = high_resolution_clock::now();
		//cout << "bootstrap takes: " << duration_cast<microseconds>(after_bs - before_bs).count() << " us" << endl;
	}

	if(rc != 0){
		std::cout << "Bootstrap with fpga failed." << std::endl;
		if(ROU_table != NULL)
			free(ROU_table);
		if(iROU_table != NULL)
			free(iROU_table);
